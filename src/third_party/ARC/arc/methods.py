...
class CVPlus:
    def __init__(self, X, Y, black_box, n_folds=5, seed=1000, verbose=False):
        ...
        if self.verbose:
            print(
                "Training black boxes on {} samples with {}-fold cross-validation:".format(
                    self.n, self.n_folds
                ),
                file=sys.stderr,
            )
            ...
            for k in tqdm(range(self.n_folds), ascii=True, disable=True):
                self.grey_boxes[k] = ProbAccum(
                    self.mu_LOO[k].predict_proba(X[test_indices[k]]).cpu().detach()
                )
        else:
            for k in range(self.n_folds):
                self.grey_boxes[k] = ProbAccum(
        ...
        if self.verbose:
            ...
            for k in tqdm(range(self.n_folds), ascii=True, disable=True):
                idx = test_indices[k]
                self.alpha_max[idx, 0] = self.grey_boxes[k].calibrate_scores(
                    Y[idx], epsilon=epsilon[idx]
                )
        else:
            for k in range(self.n_folds):
                idx = test_indices[k]
...
class JackknifePlus:
    def __init__(self, X, Y, black_box, seed=1000, verbose=True):
        ...
        if self.verbose:
            print(
                "Training black boxes on {} samples with the Jacknife+:".format(self.n),
                file=sys.stderr,
            )
            sys.stderr.flush()
        ...
        # Accumulate probabilities for the original data with the grey boxes
        self.grey_boxes = [
            ProbAccum(self.mu_LOO[i].predict_proba(X[i]).cpu().detach()) for i in range(self.n)
        ]
        ...
        if self.verbose:
            ...
            for i in range(self.n):
                print("{} of {}...".format(i + 1, self.n), file=sys.stderr)
                sys.stderr.flush()
                self.alpha_max[i, 0] = self.grey_boxes[i].calibrate_scores(
                    Y[i], epsilon=epsilon[i]
                )
        else:
            for i in range(self.n):
                self.alpha_max[i, 0] = self.grey_boxes[i].calibrate_scores(
