...
class ConformalModel(nn.Module):
    def __init__(self, model, calib_loader, alpha, kreg=None, lamda=None, randomized=True, allow_zero_sets=False, pct_paramtune = 0.3, batch_size=32, lamda_criterion='size'):
        ...
        self.num_classes = len(np.unique([calib_loader.dataset[i][1] for i in range(len(calib_loader.dataset))]))
        #self.num_classes = 10
...
def get_violation(cmodel, loader_paramtune, strata, alpha):
    ...
    for logit, target in loader_paramtune:
        # compute output
        output, S = cmodel(logit.float()) # This is a 'dummy model' which takes logits, for efficiency.
...
def get_logits_targets(model, loader):
    logits = torch.zeros((len(loader.dataset), len(np.unique([loader.dataset[i][1] for i in range(len(loader.dataset))])))) # 1000 classes in Imagenet.
