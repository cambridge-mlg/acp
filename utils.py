...
def evaluate_cp_method(model, cp_method, Xtrain, Ytrain, Xtest, Ytest, labels, out_file = None, seed = 42):
    pvals = []
    prediction_times = []
    for xtest in Xtest:
        pvals_xtest = {}
        ...
        for yhat in labels:
            pvals_xtest[yhat], scores[yhat] = cp_method(model, Xtrain, Ytrain, xtest, yhat, labels = labels, seed = seed)
...
def l2_regularization(model, l2_penalty):
    ...
    return torch.mul(l2_reg, check_tensor(l2_penalty))
...
def print_times(results_standard, results_optimized, xlim, ylim, SUPERresults_optimized = None, save_dir = None, size = 8):

    colors = sns.color_palette()

    fig, ax = plt.subplots(1, 1, figsize=(size, size))
    
    sns.lineplot(x="N", y="prediction-times", data=results_standard.groupby("N")[["prediction-times"]].mean(),
                 legend=None, label="Standard CP",
                 color=colors[0])
    sns.lineplot(x="N", y="prediction-times", data=results_optimized.groupby("N")[["prediction-times"]].mean(),
                 legend=None, label="Optimized CP (Naive)",
                 color=colors[1])
    if SUPERresults_optimized is not None:
        sns.lineplot(x="N", y="prediction-times", data=SUPERresults_optimized.groupby("N")[["prediction-times"]].mean(),
                     legend=None, label="Optimized CP (Koh)",
                     color=colors[2])
    ...
    if SUPERresults_optimized is not None:
        ax.legend(title="", loc="best", frameon=True, labels=["Standard CP", "Optimized CP (Naive)", "Optimized CP (Koh)"])
    else:
        ax.legend(title="", loc="best", frameon=True, labels=["Standard CP", "Optimized CP (Naive)"])
...
def print_pvalues(results_standard, results_optimized, xlim, ylim, label_name, label = "0", save_dir = None, size = 8): 
    diff = []
    for N, scores_s, scores_o in zip(results_standard.N, results_standard["p-values"], results_optimized["p-values"]):
...
def print_nonconformities(results_standard, results_optimized, xlim, ylim, label_name, label = "0", save_dir = None, size = 8):
    distances= []
    for N, scores_s, scores_o in zip(results_standard.N, results_standard["scores"], results_optimized["scores"]):
        distances.append(compute_distance(scores_s[label], scores_o[label], metric = "l1")/len(scores_o[label]))
    plt.figure(figsize=(size, size))
...
def print_kendall(results_standard, results_optimized, xlim, ylim, label_name, label = "0", save_dir = None, size = 8):
    tau_distances = []
    for N, pvalues_standard, pvalues_optimized in zip(results_standard.N, results_standard["p-values"], results_optimized["p-values"]):
...
def print_parameters(results_standard, results_optimized, xlim, ylim, label_name, label = "0", save_dir = None, size = 8):
    distances = []
    for N, all_params_s, all_params_o in zip(results_standard.N, results_standard["params"], results_optimized["params"]):
        aux_dist = 0
...
def plot_accum_error(epsilons, data, Ytest, file):   
    fig, ax = plt.subplots()    
    for epsilon in epsilons:
        accum_error = compute_accum_error(epsilon, data, Ytest)
        plt.step(np.arange(1, len(Ytest)+1), accum_error, label = r"$\epsilon$ = {}".format(epsilon), linewidth=3)     
    ax.legend(fontsize = "20", shadow = True, facecolor='white', framealpha=1)
    plt.xlim([0, len(Ytest)])
...
def find_min_epsilon(data, Ytest, sample, epsilons):   
    epsilons = reversed(epsilons)
    for epsilon in epsilons:
        labels = []
        for label, pvalue in data["p-values"][sample].items():
            if pvalue > epsilon:
                labels.append(label)                        
        if str(Ytest[sample]) in labels:
            return  epsilon            
        if type(Ytest[sample]) == np.ndarray:
            if str(Ytest[sample][0]) in labels:
                return epsilon            
    return epsilon
...
def get_global_coverage(data, Ytest, epsilons):   
    coverage = {}    
    for epsilon in epsilons:
        cov = 0
        for i in range(len(data["p-values"])):
            ...
            for label, pvalue in data["p-values"][i].items():
                if pvalue > epsilon:
                        labels.append(label) 
            if str(Ytest[i]) in labels:
                cov = cov + 1
            if type(Ytest[i]) == np.ndarray:
                if str(Ytest[i][0]) in labels:
                    cov = cov + 1
        coverage[epsilon] = cov/len(Ytest)
    return [x for _, x in coverage.items()]

def compute_accum_error(epsilon, data, Ytest):    
    ...
    for i in range(len(data["p-values"])):
        ...
        for label, pvalue in data["p-values"][i].items():
            if pvalue > epsilon:
                labels.append(label) 
        if type(Ytest[i]) == np.ndarray:
            if str(Ytest[i][0]) not in labels:
                accum_error[i+1] = accum_error[i] + 1
            else:
                accum_error[i+1] = accum_error[i]
        else:
            if str(Ytest[i]) not in labels:
                accum_error[i+1] = accum_error[i] + 1
            else:
                accum_error[i+1] = accum_error[i]
    return accum_error[1:]

def compute_accum_acc(epsilon, data, Ytest):    
